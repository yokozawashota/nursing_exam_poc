...
import 'record_structure_enricher.dart';
import 'domain_analysis.dart';
...
class AiAnalysisService {
  ...
  static _AnalysisStats _buildStats(
    ...
    for (final r in records) {
      final major =
      (r.major ?? '未分類').trim().isEmpty ? '未分類' : r.major!.trim();
      final mid = (r.mid ?? '未分類').trim().isEmpty ? '未分類' : r.mid!.trim();
      final topic =
      (r.topic ?? '未分類').trim().isEmpty ? '未分類' : r.topic!.trim();
